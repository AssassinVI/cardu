...
if (!$temparray[1]) {
  location_up('back','好像有東西出錯了唷，請回上一頁');
    ...
    //取出商店資料
    $store_id = $row['ns_store'];
    $row_store=pdo_select("SELECT * FROM `store` WHERE `Tb_index` LIKE '$store_id'", $where);
...
<html lang="zh-Hant-TW">
  <head>
    ...
    <meta name="keywords" content="<?php echo $row['ns_ftitle']; ?>,<?php echo $nt_name; ?>,優票證"/>  
  ...
  <body class="point_body">
    ...
    <div class="container detail_page">
        ...
        <div class="row crumbs_row">
          <div class="col-12">
            <p class="crumbs"><i class="fa fa-angle-right"></i> <a href="/index.php">首頁</a> / <a href="epoint.php">優集點</a> / <a href="javascript:;"><?php echo $nt_name?></a></p>
          </div>
        </div>
                      ...
                      <div class="cardshap ">
                        ...
                        <h2><?php echo $row['ns_ftitle'];?></h2>
                          <div class="row no-gutters my-3">
                            ...
                            <?php 
                             ...
                             //取出商店資料
                            //-- 商店銀行 --
                            $row_store=$pdo->select("SELECT * FROM store WHERE Tb_index LIKE :Tb_index", ['Tb_index'=>$row['ns_store']]);
                            $st_name=$row_store[0]['st_nickname'];
                            $store_num=count($row_store);
                            $url=empty($row_store[0]['st_main_type']) ? 'about.php?'.$row_store[0]['Tb_index'] : 'about2.php?'.$row_store[0]['Tb_index'];
                            $store_txt=$store_num==1 ? '<a target="_blank" title="'.$row_store[0]['st_nickname'].'" href="'.$url.'">
                                                       <img class="store_icon_top" src="../sys/img/'.$row_store[0]['st_logo'].'"> 
                                                     </a>' : '';
                            
                            //--------- 活動日期 ----------
                            if ($row['activity_e_date']!='0000-00-00') {
                              $activity_s_date=$row['activity_s_date']!='0000-00-00' ? $row['activity_s_date']:'即日起';
                              $activity_date='<p>活動時間： '.$activity_s_date.'~'.$row['activity_e_date'].'</p>';
                            }
                            else{
                              $activity_date='';
                            }
                            ...
                            <div class="col-md-8 col-12">
                              <?php echo $store_txt.$activity_date; ?>
                            </div>
                        ...
                        <div class="pb-3 mx-3 detail_content">
                          ...
                          <?php 
                           ...
                           <?php require('../share_area/news_other1_type2.php'); // 注恴事項 / 延伸閱讀區塊：：：：：：：：：：：：：：：：：：：：：：：?>
