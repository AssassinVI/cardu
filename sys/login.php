	...
	if (empty($admin)) {
       ...
       //------------------------- 記住帳號 -------------------------------
		...
		if ($admin['admin_per'] == 'admin') {
			location_up('module/Dashboard/index.php', '歡迎系統管理員登入');
			//登入密鑰
			login_key($admin['Tb_index']);
			$_SESSION['admin_index'] = $admin['Tb_index'];
      $_SESSION['admin_name']='系統管理員';
			$_SESSION['admin_per'] = $admin['admin_per'];
		} else {
            
            $group_txt='';
            $admin_per_arr=explode(',', $admin['admin_per']);
            $admin_per_count=count($admin_per_arr);
            for ($i=0; $i <$admin_per_count ; $i++) { 
               
              $group_where=array("Tb_index"=>$admin_per_arr[$i]);
