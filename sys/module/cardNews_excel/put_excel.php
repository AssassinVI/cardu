<?php 
  ...
  if (!empty($_POST['activity_s_date']) || !empty($_POST['activity_e_date'])) {
    $filename='活動期間'.$_POST['StartDate'].'至'.$_POST['EndDate'].'之卡情報資料.xls';
  }
  elseif (!empty($_POST['StartDate']) || !empty($_POST['EndDate'])) {
    $filename='上架期間'.$_POST['StartDate'].'至'.$_POST['EndDate'].'之卡情報資料.xls';
  }
  elseif(!empty($_POST['ns_st_vfdate']) || !empty($_POST['ns_en_vfdate'])){
    $filename='審核期間'.$_POST['StartDate'].'至'.$_POST['EndDate'].'之卡情報資料.xls';
  }
  else{
    //-- 情報狀態 --
    $ns_verify_arr=['ver_0'=>'草稿', 'ver_2'=>'審核中', 'ver_3'=>'已上架', 'ver_5'=>'退稿', 'ver_7'=>'下架'];
    $filename=$ns_verify_arr['ver_'.$_POST['ns_verify']].'卡情報資料.xls';
  }
