<?php
 ...
 if ($_POST) {
  ...
  elseif($_POST['type']=='ch_eq_content'){
      ...
      //-- 信用卡權益項目 --
      $row_card_eq=$pdo->select("SELECT * FROM debit_card_eq WHERE EndDate>=:today AND card_id=:card_id AND eq_id=:eq_id", 
  ...
  elseif($_POST['type']=='int_up_eq'){
    ...
    $row_card_eq=$pdo->select("SELECT COUNT(*) as totle FROM debit_card_eq WHERE card_id=:card_id AND eq_id=:eq_id", ['card_id'=>$_POST['card_id'], 'eq_id'=>$_POST['eq_id']], 'one' );
    ...
    if ($row_card_eq['totle']==0) {
      ...
      $pdo->insert('debit_card_eq', $param);
      echo $Tb_index;
    }
    ...
    //-- 修改 --
      ...
      $pdo->update('debit_card_eq', $param, $where);
      echo $_POST['Tb_index'];
    }
  }
  ...
  elseif($_POST['type']=='int_up_eq_schedule'){
    ...
    $row_card_eq=$pdo->select("SELECT Tb_index, COUNT(*) as totle FROM debit_card_eq WHERE card_id=:card_id AND eq_id=:eq_id", ['card_id'=>$_POST['card_id'], 'eq_id'=>$_POST['eq_id']], 'one' );
    ...
    if ($row_card_eq['totle']==1) {
      ...
      $pdo->update('debit_card_eq', ['EndDate'=>date('Y-m-d',strtotime($_POST['StartDate']))], ['Tb_index'=>$row_card_eq['Tb_index']]);
      echo $Tb_index;
    }
