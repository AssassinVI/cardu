...
if ($_GET) {
  ...
  $parent_mt_id=pdo_select("SELECT parent_id FROM maintable WHERE Tb_index=:Tb_index", ['Tb_index'=>$_GET['MT_id']]);
   ...
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT * FROM appNews WHERE mt_id=:mt_id AND ns_verify=2  ORDER BY OrderBy DESC, StartDate DESC");
   $sql->execute(array( "mt_id"=>$parent_mt_id['parent_id']));   
}
