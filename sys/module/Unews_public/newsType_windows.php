				...
				<div class="panel-body">
				 <form class="put_form">
				  ...
				  <div  class="type_div">
				  	...
				  	<div class="type">
				  	  <?php 
                        $cardNews_type=$NewPdo->select("SELECT * FROM news_type WHERE mt_id='site2019011115561564' ORDER BY OrderBy ASC");
                        foreach ($cardNews_type as $cardNews_type_one) {
                        	echo '<label><input type="checkbox" name="ns_nt_ot_pk[]" typeName="'.$cardNews_type_one['nt_name'].'" value="'.$cardNews_type_one['Tb_index'].'"> '.$cardNews_type_one['nt_name'].'</label>｜';
                        }
				  	  ?>
				  	</div>
				  </div>
