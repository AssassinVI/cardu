                     ...
                     <form id="search_ns_form" class="form-horizontal" method="POST" action="admin_list.php?MT_id=<?php echo$_GET['MT_id']; ?>">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-10 form-inline">
                          ...
                          <select class="form-control" name="st_main_type" style="display: none">
                            <option value="">全部</option>

                          </select>

                          <select class="form-control" name="st_second_type" style="display: none">
                            <option value="">全部</option>

                          </select>

                          <select class="form-control" name="ns_store" style="display: none">
                            <option value="">全部</option>
                      ...
                      <div class="form-group">
                        <div class="col-md-12 text-right">
                            <button type="button" id="search_btn" class="btn btn-success btn-raised">查看</button>
                            <button type="button" id="down_btn" class="btn btn-success btn-raised">下載</button>
                        </div>
                      </div>
                     </form>
                 </div>
...
<script type="text/javascript">
	$(document).ready(function() {
     ...
     $('#search_btn').click(function(event) {
      $('#search_ns_form').attr('action', 'show_excel.php');
      $('#search_ns_form').attr('target', 'TheWindows');
      window.open('', 'TheWindows', config='height=800,width=1000');
      $('#search_ns_form').submit();
     });

     $('#down_btn').click(function(event) {
      $('#search_ns_form').attr('target', '');
      $('#search_ns_form').attr('action', 'put_excel.php');
      $('#search_ns_form').submit();
     });
     ...
     $('[name="st_type"]').change(function(event) {
      ...
      $('[name="st_main_type"]').html('<option value="">全部</option>');
      $('[name="st_second_type"]').html('<option value="">全部</option>');
      $('[name="ns_store"]').html('<option value="">全部</option>');
       ...
       if ($(this).val()!='') {
        ...
        if ($(this).val()=='st2019013117015133' || $(this).val()=='st201901311701582') {
          ...
          $('[name="st_main_type"]').css('display', 'inline-block');
          $('[name="st_second_type"]').css('display', 'inline-block');
          $('[name="ns_store"]').css('display', 'inline-block');
          ...
          $.ajax({
            ...
            success:function (data) {
              $.each(data, function(index, val) {
                 $('[name="st_main_type"]').append('<option value="'+this['Tb_index']+'">'+this['st_name']+'</option>');
              });
            }
          });
          ...
          $('[name="st_main_type"]').css('display', 'none');
          $('[name="st_second_type"]').css('display', 'none');
          $('[name="ns_store"]').css('display', 'inline-block');
          ...
          $.ajax({
            ...
            success:function (data) {
              $.each(data, function(index, val) {
                 $('[name="ns_store"]').append('<option value="'+this['Tb_index']+'">'+this['st_name']+'</option>');
              });
            }
          });
        }
         ...
         $('[name="st_main_type"]').css('display', 'none');
         $('[name="st_second_type"]').css('display', 'none');
         $('[name="ns_store"]').css('display', 'none');
       }
     });


     //-- 選擇商店主分類 --
     $('[name="st_main_type"]').change(function(event) {
        ...
        $.ajax({
          ...
          success:function (data) {
            $.each(data, function(index, val) {
                 $('[name="st_second_type"]').append('<option value="'+this['Tb_index']+'">'+this['st_name']+'</option>');
              });
          }
        });
     });
     ...
     //-- 選擇商店次分類 --
     $('[name="st_second_type"]').change(function(event) {
        ...
        $.ajax({
          ...
          data: {
            type: 'store',
            st_type:$('[name="st_type"]').val(),
            st_main_type:$('[name="st_main_type"]').val(),
