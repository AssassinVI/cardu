...
if ($_GET) {
   ...
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT a.Tb_index, a.attr_name, a.OnLineOrNot, COUNT(a.Tb_index) as total
                       FROM card_attr as a 
                       LEFT JOIN member_card as mc ON FIND_IN_SET(a.Tb_index, mc.card_attr)
                       WHERE mt_id=:mt_id  
                       GROUP BY a.Tb_index
				...
				<div class="table-responsive">
					<table class="table table-hover">
						...
						<?php $i=1; while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            $OnLineOrNot=$row['OnLineOrNot']=='1' ? '使用中':'<span class="text-danger">未使用</span>';
					    ?>
							<tr>
								<td><?php echo $i?></td>
