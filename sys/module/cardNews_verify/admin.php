...
if ($_GET) {
  ...
  foreach ($area_row as $area_one) {
   ...
   $row=$NewPdo->select("
   	 ...
   	 WHERE news.mt_id=:mt_id AND news.ns_verify=2 
					...
					<table class="table no-margin">
						...
						<?php 
						     ...
						     foreach ($row as $row_one){
						      ...
						      //-- 情報銀行 --
						      $where=['news_id'=>$row_one['Tb_index']];
						      ...
						      $ns_nt_pk=pdo_select("SELECT area_id, nt_name FROM news_type WHERE Tb_index=:Tb_index", ['Tb_index'=>$row_one['ns_nt_pk']]);
							...
							<tr>
								...
								<td><?php echo $row_one['ns_ftitle']; ?></td>
								<td><?php echo $ns_verify;?></td>
								<td><?php echo $row_bank['bi_shortname']; ?></td>
								<td><?php echo $row_one['ns_fwdate'];?></td>
								<td><?php echo $row_one['ns_reporter'];?></td>
						
								<td >
								<a href="javascript:;" onclick="window.open('../cardNews_public/newsView_windows.php?Tb_index=<?php echo $row_one['Tb_index'];?>', '<?php echo $row_one['ns_ftitle']; ?>', config='height=800,width=900');" >
								...
								<td>
									<a href="manager_verify.php?MT_id=<?php echo $_GET['MT_id']?>&Tb_index=<?php echo $row_one['Tb_index'];?>" >
									<i class="fa fa-file-text" aria-hidden="true"></i>
									審核
									</a>
								</td>
								<td>
									<a href="manager.php?MT_id=<?php echo $_GET['MT_id']?>&Tb_index=<?php echo $row_one['Tb_index'];?>" >
