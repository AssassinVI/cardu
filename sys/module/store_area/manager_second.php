...
if ($_POST) {
  ...
  $OrderBy=pdo_select("SELECT OrderBy FROM store_type_second WHERE mt_id=:mt_id ORDER BY OrderBy DESC LIMIT 0,1", ['mt_id'=>$_POST['second_id']]);
  $OrderBy=(int)$OrderBy['OrderBy']+1;
 

	$param=  ['Tb_index'=>$Tb_index,
			     'mt_id'=>$_POST['second_id'],
			   'st_name'=>$_POST['st_name'],
               'remark'=>$_POST['remark'],
               'OrderBy'=>$OrderBy,

	];
	pdo_insert('store_type_second', $param);
	location_up('admin_second.php?MT_id='.$_POST['mt_id'].'&second_id='.$_POST['second_id'],'成功新增');
   }
   else{  //修改
	...
	pdo_update('store_type_second', $param, $where);
	
	location_up('admin_second.php?MT_id='.$_POST['mt_id'].'&second_id='.$_POST['second_id'],'成功更新');
   }
}
if ($_GET) {

	//-- 主分類 --
    $store_type_main=pdo_select("SELECT st_name FROM store_type_main WHERE Tb_index=:Tb_index ", ["Tb_index"=>$_GET['second_id']]);

 	$where=['Tb_index'=>$_GET['Tb_index']];
 	$row=pdo_select('SELECT * FROM store_type_second WHERE Tb_index=:Tb_index', $where);
		...
		<div class="col-lg-9">
			<div class="panel panel-default">
				<div class="panel-heading">
					<header><?php echo $store_type_main['st_name']; ?> 主分類編輯
					</header>
				</div><!-- /.panel-heading -->
				<div class="panel-body">
					<form id="put_form" action="manager_second.php" method="POST" enctype='multipart/form-data' class="form-horizontal">
						...
						<input type="hidden" id="second_id" name="second_id" value="<?php echo $_GET['second_id'];?>">
					</form>
				</div><!-- /.panel-body -->
			</div><!-- /.panel -->
