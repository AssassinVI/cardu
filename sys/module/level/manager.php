  ...
  // ======================== 刪除 ===========================
    ...
    if (!empty($_POST['type']) && $_POST['type']=='delete') { 
        ...
        //----------------------- 多檔刪除 -------------------------------
    		...
    		$otr_file=pdo_select('SELECT OtherFile FROM card_level WHERE Tb_index=:Tb_index', $sel_where);
  ...
  $OnLineOrNot=empty($_POST['OnLineOrNot']) ? 0:1;
	...
	location_up('admin.php?MT_id='.$_POST['mt_id'],'成功新增');
      ...
      if (!empty($_FILES['aPic']['name'])) {
        ...
        pdo_update('card_level', $aPic_param, $aPic_where);

      }
      //-------------------- 多檔上傳 ------------------------------
      if (!empty($_FILES['OtherFile']['name'][0])) {
      	$sel_where=['Tb_index'=>$Tb_index];
      	$now_file =pdo_select("SELECT OtherFile FROM card_level WHERE Tb_index=:Tb_index", $sel_where);
        ...
        pdo_update('card_level', $OtherFile_param, $OtherFile_where);
      }
      	//--------------------------- END -----------------------------------
