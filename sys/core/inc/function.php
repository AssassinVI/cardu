<?php  require_once 'config.php';
 ...
 function pdo_delete($tb_name, $where)
 {
  ...
  for ($i=0; $i <count($where) ; $i++) { 
    if ($i==count($where)-1) {
      $where_data.=$where_key[$i].'=:'.$where_key[$i];
    }else{
      $where_data.=$where_key[$i].'=:'.$where_key[$i].' AND ';
    }
  }
    
    $sql_query="DELETE FROM ".$tb_name." WHERE ".$where_data;
 ...
 function pdo_select($sql_query, $where)
 {
  ...
  /* ----------------------- PDO 查詢 NEW --------------------------- */
 function pdo_select_NEW($sql_query, $where, $type='all')
 {
   $pdo=pdo_conn();
   $sql=$pdo->prepare($sql_query);
