            ...
            <ul class="nav metismenu" id="side-menu">
                <li class="nav-header">
                    <div class="dropdown profile-element">
                            ...
                            <div id="cLogoWord"><?php echo $company['name'] ?>｜管理介面</div>
                            <div><span style="color: #fff;">使用者：<?php echo $_SESSION['admin_name'];?></span></div>
                    </div>
...
$sql_query = "SELECT *, mt.Tb_index as mt_Tb_index
                             ...
                             WHERE mt.parent_id='' AND mt.weblang=:weblang AND OnLineOrNot='1' ORDER BY OrderBy ASC,mt.Tb_index ASC";
$pdo = pdo_conn();
...
while ($bar = $sql->fetch(PDO::FETCH_ASSOC)) {
	...
	// ------------------------- 資料夾 -------------------------------------
        ...
        $active_css=$parent_id['parent_id']==$bar['mt_Tb_index'] ? 'active' : '';
		...
		$sql_query = "SELECT *, mt.Tb_index as mt_Tb_index
                                        FROM maintable as mt left JOIN sysModule as md ON mt.UseModuleID=md.Tb_index
                                        WHERE mt.parent_id=:parent_id AND mt.weblang=:weblang AND OnLineOrNot='1' ORDER BY OrderBy ASC,mt.Tb_index ASC";
...
function bar_tree_tb($Tb_index, $Mod_code, $MT_Name, $is_data) {
	if ($is_data == '1') {
		...
		$sql_query = "SELECT *, mt.Tb_index as mt_Tb_index
                         FROM maintable as mt left JOIN sysModule as md ON mt.UseModuleID=md.Tb_index
                         WHERE parent_id=:parent_id AND OnLineOrNot='1' ORDER BY OrderBy ASC,mt.Tb_index ASC";
		$pdo = pdo_conn();
		$sql = $pdo->prepare($sql_query);
